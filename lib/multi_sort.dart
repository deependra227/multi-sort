...
class MultiSort{
  ...
  dynamic sortingList;
  bool increasing;
  MultiSort({@required this.allProperties, @required this.preferrence, @required this.sortingList, this.increasing = false});

  List<dynamic> sort(){
    int compare(String property, dynamic a, dynamic b){
        if(a.get(property) == b.get(property)) return 0;
        else if(a.get(property) > b.get(property)) return increasing ? 1 : -1;
        else return increasing ? -1 : 1;
    }
